...
function sendWhatsAppMessages() {
    ...
    // Create combined message for boss
    const bossMessage = createBossMessage(accountsMessage, logisticsMessage);
    ...
    Promise.all([
        ...
        sendWhatsAppMessage(ENV_CONFIG.WHATSAPP_PHONE_BOSS1, bossMessage, 'boss1'),
        sendWhatsAppMessage(ENV_CONFIG.WHATSAPP_PHONE_BOSSOG, bossMessage, 'bossog')
   ]).then(results => {
       const successCount = results.filter(r => r !== null).length;
       if (successCount === 4) {
           showNotification('WhatsApp messages sent successfully!', 'success');
       } else if (successCount >= 1) {
           showNotification(`${successCount} WhatsApp message(s) sent, ${4-successCount} failed. Check console for details.`, 'warning');
       } else {
...
// Create combined message for boss
function createBossMessage(accountsMessage, logisticsMessage) {
...
function initializeWhatsApp() {
   ...
   if (validation.isValid) {
       ...
       console.log(`📞 Boss: ${ENV_CONFIG.WHATSAPP_PHONE_BOSS}`);
